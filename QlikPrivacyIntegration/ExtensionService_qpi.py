...
import grpc
from SSEData_qpi import FunctionType
from ScriptEval_qpi import ScriptEval
...
config_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'qpi.config')
...
def csvlogger(rows):
    ...
    auditFile =  auditLogPath + '/' + configAudit.get('fileNamePrefix','QPI_Audit')

    #If the server name is requested to be added, include it here.
    if (configAudit.getboolean('fileNameIncludeServerName',1)):
        auditFile = auditFile + '_' + platform.node()

    #Get TimeStamp pattern, format it, and add to AuditFile
    auditTSPattern = '%' + '_%'.join(configAudit.get('fileNameTSPattern','YmdH'))
    auditFile = auditFile + '_' + datetime.now().strftime(auditTSPattern) + '.csv'  
        ...
        for request_rows in request:
            ...
            for row in request_rows.rows:
                ...
                try:
                    ...
                    decryptedValue = 'Can not decrypt data, see log file. DEBUG ONLY: ' + [d.strData for d in row.duals][0]
